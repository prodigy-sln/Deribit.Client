...
public class DeribitAuthenticationClient
{
    ...
    public DeribitAuthenticationClient(DeribitJsonRpcClient deribitJsonRpcClient, RequestSignatureGenerator signatureGenerator, DeribitAuthenticationSession authSession, IOptions<DeribitClientOptions> options)
    {
    ...
    public async Task<AuthResponse?> AuthenticateAsync(AuthRequest request)
    {
        ...
        switch (request.GrantType)
        {
            case AuthRequestGrantType.ClientCredentials:
                ...
                requestObject = new { grant_type = "client_credentials", client_id = _options.Value.ClientId, client_secret = _options.Value.ClientSecret, timestamp = timestamp, state = request.State, scope = request.Scope };
                break;
            case AuthRequestGrantType.ClientSignature:
                ...
                requestObject = new { grant_type = "client_signature", client_id = _options.Value.ClientId, signature = signatureInfo.Signature, nonce = signatureInfo.Nonce, data = signatureInfo.Data, timestamp = timestamp, state = request.State, scope = request.Scope };
                break;
            case AuthRequestGrantType.RefreshToken:
                if (string.IsNullOrWhiteSpace(request.RefreshToken))
                    throw new ArgumentNullException(nameof(request.RefreshToken));
                
                requestObject = new { grant_type = "refresh_token", client_id = _options.Value.ClientId, refresh_token = request.RefreshToken, timestamp = timestamp, state = request.State, scope = request.Scope };
                break;
            default:
