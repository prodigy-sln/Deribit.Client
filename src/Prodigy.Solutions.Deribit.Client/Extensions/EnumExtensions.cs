...
public static class EnumExtensions<T> where T : struct, Enum
{
    ...
    {
        if (!EnumNames.TryGetValue(EnumType, out var names)) names = EnumNames[EnumType] = new Dictionary<T, string>();

        if (names.TryGetValue(value, out var serialized)) return serialized;
